...
const CustomTable = ({
    ...
    const printColumns = visibleColumns.filter(col => !col.isAction && col.type !== 'switch');
    ...
    const renderActionButtons = (row: TableRow) => (
        <div className="flex items-center space-x-2 justify-center print:hidden">
            {actions.map((action) => {
                ...
                const isAllowed = hasPermission(action.permission, user?.permissions ?? []);
                ...
                return (
                    <Link key={`${action.key}-${row.id}`} title={action.tooltip} to={`${action.route}/${row.id}`}>
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg hover:shadow-xl print:bg-gray-800 print:shadow-none">
                                ...
                                {(typeof window !== 'undefined' && window.matchMedia('print').matches ? printColumns : displayColumns).map((column) => (
                                    <th
                                        key={column.key}
                                        className={`px-4 py-3 text-sm font-medium text-white cursor-pointer transition-all duration-200 print:cursor-default print:bg-gray-800 ${column.sortable && typeof window !== 'undefined' && !window.matchMedia('print').matches
                                            ? 'hover:bg-blue-500'
                                            : ''
                                            } ${column.className || ''} ${column.isAction ? 'print:hidden' : ''} ${column.type === 'switch' ? 'print:hidden' : ''}`}
                                        onClick={() => typeof window !== 'undefined' && !window.matchMedia('print').matches && handleSortClick(column)}
                                    >
                                        <div className="flex items-center justify-center space-x-1">
                                            <span>{column.label}</span>
                                            {/* Sort Icon - only if column is sortable and not printing */}
                                            {column.sortable && typeof window !== 'undefined' && !window.matchMedia('print').matches && (
                                                <>
                        ...
                        <tbody className="divide-y divide-blue-100 print:divide-gray-200">
                            {data.map((row, index) => (
                                <tr key={index} className="hover:bg-blue-50 transition-colors duration-150 print:hover:bg-transparent print:border-b print:border-gray-300">
                                    {/* Checkbox column */}
                                    {enableSelection && (
                                    ...
                                    {displayColumns.map((column) => (
                                        <td
                                            ...
                                            className={`px-6 py-2 text-sm text-gray-700 text-center print:px-2 print:py-1 print:text-xs ${column.className} ${column.isAction ? 'print:hidden' : ''} ${column.type === 'switch' ? 'print:hidden' : ''}`}
                                        >
                                            {column.isAction ? (
                                                ...
                                                <div className="print:hidden">
                                                    <Switch
                                                        checked={row.status === 'active' || row.required === 'yes'}
                                                        onCheckedChange={(checked) => onStatusToggle?.(row.id, checked)}
                                                        className={`h-4 w-10 ${row.status === 'active' || row.required === 'yes'
                                                ...
                                                <div className="flex flex-wrap justify-center items-center gap-1">
                                                    {row[column.key].map((permission: any) => (
                                                        <Badge key={`${row.id}-${permission.id}`} variant="destructive" className="print:bg-gray-200 print:text-gray-800 print:border print:border-gray-400">
                                                            {permission.label}
                                                        </Badge>
                                                    ))}
                                                </div>
