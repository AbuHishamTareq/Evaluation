...
export function CustomModelForm({
            ...
            case 'multi-select':
                return (
                    <MultiSelect
                        ...
                        variant="inverted"
                        maxCount={7}
                        defaultValue={Array.isArray(formData[field.name]) ? formData[field.name] : []}
                        readOnly={readOnly}
                    />
                );
            case 'file':
    ...
    const showAddButton = !addButton.permission || (user?.permissions && hasPermission(addButton.permission, user.permissions));
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange} modal>
            ...
            <DialogContent onInteractOutside={(e) => e.preventDefault()} className="sm:max-w-[830px] border-cyan-200 bg-gradient-to-br from-cyan-100 to-blue-400">
                <form onSubmit={handleSubmit}>
                    <DialogHeader className="border-b border-cyan-200 pb-4">
                        <DialogTitle className="text-cyan-800 text-xl font-semibold">{title}</DialogTitle>
                        <DialogDescription className="text-cyan-600">{description}</DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter className="gap-2 mt-4 pt-4 border-t border-cyan-200">
                        <DialogClose asChild>
                            <Button key="cancel-btn" id="cancel-btn" label="" type="button" variant="outline" className="bg-red-500 hover:bg-red-600 text-white shadow-lg hover:shadow-xl">
                                Cancel
                            </Button>
                        </DialogClose>

                        {mode !== "view" && buttons.map((button) => (
                            <Button
                                label={button.label}
                                key={button.key}
                                id={button.id}
