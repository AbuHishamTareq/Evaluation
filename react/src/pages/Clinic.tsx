...
import { ClinicsTableConfig } from "@/config/tables/clinic-table";
import api from "../axios";
import { CustomModelForm } from "../components/CustomModelForm";
import { ClinicsModelFormConfig } from "@/config/forms/clinic-model-form";
...
export const Clinic = () => {
  ...
  const canPrint = hasPermission(`print-clinic`, userPermissions);
  const canExport = hasPermission(`export-clinic`, userPermissions);
  const canImport = hasPermission(`import-clinic`, userPermissions);
  ...
  const fetchAllClinicsForPrint = async (): Promise<any[]> => {
    try {
      ...
      const fetchedClinics = res.data.clinics.data;
  ...
  const handlePrintClick = async () => {
    ...
    try {
      ...
      const fetchedClinics = await fetchAllClinicsForPrint();
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: "All Clinics Report",
    onAfterPrint: () => {
      setAllClinics([]);
    },
  });
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      const response = await api.post("/api/clinics/import", formData, {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await api.get("/api/clinics/download-template", {
        responseType: "blob",
      });
  ...
  const fetchClinics = async (
    ...
    try {
      ...
      const clinicData = res.data.clinics;
      setClinics(clinicData.data);

      setPagination({
        current_page: clinicData.current_page,
        last_page: clinicData.last_page,
        per_page: clinicData.per_page,
      });
    } catch {
  ...
  // Fetch all clinics data for export
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchClinics(newPage, pagination.per_page, search, sortBy, sortDir);
  };

  const handlePerPageChange = (newPerPage: number) => {
    fetchClinics(1, newPerPage, search, sortBy, sortDir);
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast({
        title: "Deleted",
        description:
          response.data?.message || "Clinics deleted successfully.",
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
      await fetchClinics(); // refresh the data
    } catch (error: any) {
      toast({
        title: "Error",
        description:
  ...
  const openModel = (mode: "create" | "view" | "edit", clinic?: any) => {
    setMode(mode);
    setSelectedCenter(clinic || null);
    setModelOpen(true);
  };

  const getFormFieldsWithValues = () => {
    return ClinicsModelFormConfig.fields.map((field) => {
      let value = "";
  ...
  const handleBulkExportCSV = () => {
    ...
    const selectedClinic = clinics.filter((clinic) =>
      selectedRows.includes(clinic.id)
    );
    const headers = ClinicsTableConfig.columns
      .filter((col) => !["actions", "status"].includes(col.key))
      .map((col) => col.label);

    const rows = selectedClinic.map((clinic) =>
      ClinicsTableConfig.columns
        .filter((col) => !["actions", "status"].includes(col.key))
        .map((col) => {
          const value = clinic[col.key];
  ...
  const handleBulkExportPDF = () => {
    ...
    const visibleColumns = ClinicsTableConfig.columns.filter(
  ...
  const handleExportAllCSV = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `all_healthcare_fields_${allClinics.length}.csv`
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      ...
      toast({
        title: "Success",
        description: `Exported ${allClinics.length} healthcare fields to CSV.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `clinics_page_${pagination.current_page}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    toast({
      title: "Success",
      description: `Exported ${clinics.length} fields from current page to CSV.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleBulkExportExcel = async () => {
    ...
    const selectedDomains = clinics.filter((clinic) =>
      selectedRows.includes(clinic.id)
    );
    ...
    // Add data
    selectedDomains.forEach((clinic) => {
      worksheet.addRow(clinic);
    });
    ...
    saveAs(blob, `selected_clinics_${selectedRows.length}.xlsx`);
  };

  const handleExportAllExcel = async () => {
    try {
      const allDomains = await fetchAllClinics();
      ...
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("All Clinics");

      // Auto-generate columns from first clinic object
      ...
      // Add each clinic as a row
      allDomains.forEach((clinic: any) => {
        worksheet.addRow(clinic);
      });
      ...
      saveAs(blob, `all_clinics_${allDomains.length}.xlsx`);

      toast({
        title: "Success",
        description: `Exported ${allDomains.length} clinics to Excel.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to export clinics.",
        backgroundColor: "bg-red-600",
        color: "text-white",
        variant: "destructive",
      });
  ...
  const handleCurrentPageExportExcel = async () => {
    ...
    saveAs(blob, `clinics_page_${pagination.current_page}.xlsx`);

    toast({
      title: "Success",
      description: `Exported ${clinics.length} fields from current page to Excel.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleExportAllPDF = async () => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `All Clinics Report (${allClinics.length} clinics)`,
        pageWidth / 2,
        20,
        {
          align: "center",
      ...
      doc.save(`all_clinics_${allClinics.length}.pdf`);

      toast({
        title: "Success",
        description: `Exported ${allClinics.length} healthcare fields to PDF.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportPDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `Clinics Report - Page ${pagination.current_page} (${clinics.length} clinics)`,
      pageWidth / 2,
      20,
      { align: "center" }
    );
    ...
    toast({
      title: "Success",
      description: `Exported ${clinics.length} healthcare fields from current page to PDF.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            ...
            {isImportOpen && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                ...
                <div className="mt-2 text-sm text-green-600">
                  Upload CSV or Excel files with columns: en_clinic,
                  ar_clinic.
                </div>
              </div>
            )}
            ...
            <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <CustomModelForm
                addButton={ClinicsModelFormConfig.addButton}
                title={
                  mode === "view"
                    ? "View Clinic"
                    : mode === "edit"
                    ? "Update Clinic"
                    : ClinicsModelFormConfig.title
                }
                description={ClinicsModelFormConfig.description}
                fields={getFormFieldsWithValues()}
                buttons={ClinicsModelFormConfig.buttons}
                onSubmit={async (data, setErrors) => {
                  try {
                    await ClinicsModelFormConfig.onSubmit(
                      data,
                      mode,
                      selectedCenter?.id,
                      setErrors
            ...
            <div ref={printRef} className="hidden print:block">
              ...
              <div className="flex justify-between items-center border-b pb-4 mb-4">
                ...
                <div className="text-center w-full -ml-16">
                  <h1 className="text-2xl font-bold">Clinics Report</h1>
            ...
            <div className="print:hidden">
              <CustomTable
                columns={ClinicsTableConfig.columns}
                actions={ClinicsTableConfig.actions}
                data={clinics}
                ...
                onView={(clinic) => openModel("view", clinic)}
                onEdit={(clinic) => openModel("edit", clinic)}
                isModel={true}
                sortBy={sortBy}
                sortDir={sortDir}
