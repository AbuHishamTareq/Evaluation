...
import { HcRolesTableConfig } from "@/config/tables/hcRole-table";
import api from "../axios";
import { CustomModelForm } from "../components/CustomModelForm";
import { HcRolesModelFormConfig } from "@/config/forms/hcRole-model-form";
...
export const HcRole = () => {
  const [hcRoles, setHcRoles] = useState<any[]>([]);
  ...
  const canPrint = hasPermission(`print-healthcare-role`, userPermissions);
  const canExport = hasPermission(`export-healthcare-role`, userPermissions);
  const canImport = hasPermission(`import-healthcare-role`, userPermissions);

  const printRef = useRef<HTMLDivElement>(null);
  const [allHcRoles, setAllHcRoles] = useState<any[]>([]);
  const [isLoadingAll, setIsLoadingAll] = useState(false);

  // Fetch all users data for printing - Fixed version
  const fetchAllHcRolesForPrint = async (): Promise<any[]> => {
    try {
      const res = await api.get(`/api/healthcareRoles`, {
        params: {
          page: 1,
      ...
      const fetchedHcRoles = res.data.hcRoles.data;
      return fetchedHcRoles;
    } catch (error) {
  ...
  const handlePrintClick = async () => {
    ...
    try {
      ...
      const fetchedHcRoles = await fetchAllHcRolesForPrint();

      if (fetchedHcRoles.length === 0) {
        toast({
          title: "Warning",
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: "All HcRoles Report",
    onAfterPrint: () => {
      setAllHcRoles([]);
    },
  });
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      const response = await api.post("/api/healthcareRoles/import", formData, {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await api.get("/api/healthcareRoles/download-template", {
        responseType: "blob",
      });
  ...
  const fetchHcRoles = async (
    ...
    try {
      ...
      const hcRoleData = res.data.hcRoles;
      setHcRoles(hcRoleData.data);

      setPagination({
        current_page: hcRoleData.current_page,
        last_page: hcRoleData.last_page,
        per_page: hcRoleData.per_page,
      });
    } catch {
  ...
  // Fetch all hcRoles data for export
  const fetchAllHcRoles = async () => {
    try {
      const res = await api.get(`/api/healthcareRoles`, {
        params: {
          page: 1,
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchHcRoles(newPage, pagination.per_page, search, sortBy, sortDir);
  };

  const handlePerPageChange = (newPerPage: number) => {
    fetchHcRoles(1, newPerPage, search, sortBy, sortDir);
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await api.delete(`/api/healthcareRoles/delete/${id}`);
      toast({
        title: "Deleted",
        description: response.data?.message || "HcRoles deleted successfully.",
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
      await fetchHcRoles(); // refresh the data
    } catch (error: any) {
      toast({
        title: "Error",
        description:
  ...
  const openModel = (mode: "create" | "view" | "edit", hcRole?: any) => {
    setMode(mode);
    setSelectedCenter(hcRole || null);
    setModelOpen(true);
  };

  const getFormFieldsWithValues = () => {
    return HcRolesModelFormConfig.fields.map((field) => {
      let value = "";
  ...
  const handleBulkExportCSV = () => {
    ...
    const selectedHcRole = hcRoles.filter((hcRole) =>
      selectedRows.includes(hcRole.id)
    );
    const headers = HcRolesTableConfig.columns
      .filter((col) => !["actions", "status"].includes(col.key))
      .map((col) => col.label);

    const rows = selectedHcRole.map((hcRole) =>
      HcRolesTableConfig.columns
        .filter((col) => !["actions", "status"].includes(col.key))
        .map((col) => {
          const value = hcRole[col.key];
  ...
  const handleBulkExportPDF = () => {
    ...
    const visibleColumns = HcRolesTableConfig.columns.filter(
  ...
  const handleExportAllCSV = async () => {
    try {
      const allHcRoles = await fetchAllHcRoles();

      if (allHcRoles.length === 0) {
        toast({
          title: "Error",
          description: "You must select at least one Record.",
          backgroundColor: "bg-red-600",
      ...
      const rows = allHcRoles.map((hcRole: any) =>
        HcRolesTableConfig.columns
          .filter((col) => !["actions", "status"].includes(col.key))
          .map((col) => {
            const value = hcRole[col.key];
      ...
      link.setAttribute(
        "download",
        `all_healthcare_fields_${allHcRoles.length}.csv`
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      ...
      toast({
        title: "Success",
        description: `Exported ${allHcRoles.length} healthcare fields to CSV.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `healthcareRoles_page_${pagination.current_page}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    toast({
      title: "Success",
      description: `Exported ${hcRoles.length} fields from current page to CSV.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleBulkExportExcel = async () => {
    ...
    const selectedDomains = hcRoles.filter((hcRole) =>
      selectedRows.includes(hcRole.id)
    );
    ...
    // Add data
    selectedDomains.forEach((hcRole) => {
      worksheet.addRow(hcRole);
    });
    ...
    saveAs(blob, `selected_healthcareRoles_${selectedRows.length}.xlsx`);
  };

  const handleExportAllExcel = async () => {
    try {
      const allDomains = await fetchAllHcRoles();
      ...
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("All HcRoles");

      // Auto-generate columns from first hcRole object
      ...
      // Add each hcRole as a row
      allDomains.forEach((hcRole: any) => {
        worksheet.addRow(hcRole);
      });
      ...
      saveAs(blob, `all_healthcareRoles_${allDomains.length}.xlsx`);

      toast({
        title: "Success",
        description: `Exported ${allDomains.length} healthcare Roles to Excel.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to export healthcare Roles.",
        backgroundColor: "bg-red-600",
        color: "text-white",
        variant: "destructive",
      });
  ...
  const handleCurrentPageExportExcel = async () => {
    ...
    saveAs(blob, `healthcareRoles_page_${pagination.current_page}.xlsx`);

    toast({
      title: "Success",
      description: `Exported ${hcRoles.length} fields from current page to Excel.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleExportAllPDF = async () => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `All HcRoles Report (${allHcRoles.length} healthcareRoles)`,
        pageWidth / 2,
        20,
        {
          align: "center",
      ...
      doc.save(`all_healthcareRoles_${allHcRoles.length}.pdf`);

      toast({
        title: "Success",
        description: `Exported ${allHcRoles.length} healthcare fields to PDF.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportPDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `HcRoles Report - Page ${pagination.current_page} (${hcRoles.length} healthcare Roles)`,
      pageWidth / 2,
      20,
      { align: "center" }
    );
    ...
    toast({
      title: "Success",
      description: `Exported ${hcRoles.length} healthcare fields from current page to PDF.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  return (
    <Header title={"Healthcare Roles & Administration Management"}>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-blue-800">
                    {selectedRows.length} Healthcare Roles & Administration
                    selected
                  </span>
                </div>
            ...
            {isImportOpen && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                ...
                <div className="mt-2 text-sm text-green-600">
                  Upload CSV or Excel files with columns: en_hcRole, ar_hcRole.
                </div>
              </div>
            )}
            ...
            <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <CustomModelForm
                addButton={HcRolesModelFormConfig.addButton}
                title={
                  mode === "view"
                    ? "View HcRole"
                    : mode === "edit"
                    ? "Update HcRole"
                    : HcRolesModelFormConfig.title
                }
                description={HcRolesModelFormConfig.description}
                fields={getFormFieldsWithValues()}
                buttons={HcRolesModelFormConfig.buttons}
                onSubmit={async (data, setErrors) => {
                  try {
                    await HcRolesModelFormConfig.onSubmit(
                      data,
                      mode,
                      selectedCenter?.id,
                      setErrors
            ...
            <div ref={printRef} className="hidden print:block">
              ...
              <div className="flex justify-between items-center border-b pb-4 mb-4">
                ...
                <div className="text-center w-full -ml-16">
                  <h1 className="text-2xl font-bold">HcRoles Report</h1>
              ...
              {allHcRoles.length === 0 && (
                <div className="text-center py-8">
                  <p className="text-gray-600">
                    No healthcare Roles & Administration found to display.
                  </p>
                </div>
              )}
            ...
            <div className="print:hidden">
              <CustomTable
                columns={HcRolesTableConfig.columns}
                actions={HcRolesTableConfig.actions}
                data={hcRoles}
                ...
                onView={(hcRole) => openModel("view", hcRole)}
                onEdit={(hcRole) => openModel("edit", hcRole)}
                isModel={true}
                sortBy={sortBy}
                sortDir={sortDir}
