...
const Users = () => {
    ...
    try {
      ...
      const message =
        updated === total
          ...
          : `${updated} of ${total} selected user${
              total > 1 ? "s" : ""
  ...
  const handleBulkExportPDF = () => {
    ...
    const visibleColumns = UsersTableConfig.columns.filter(
    ...
    doc.text(
      `Selected Users Report (${selectedRows.length} users)`,
      pageWidth / 2,
      20,
      { align: "center" }
    );
  ...
  const handleBulkExportExcel = async () => {
    ...
    const selectedDomains = users.filter((user) =>
      selectedRows.includes(user.id)
    );
  ...
  const handleExportAllPDF = async () => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `All Users Report (${allUsers.length} users)`,
        pageWidth / 2,
        20,
        { align: "center" }
      );
  ...
  const handleCurrentPageExportPDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `Users Report - Page ${pagination.current_page} (${users.length} users)`,
      pageWidth / 2,
      20,
      { align: "center" }
    );
            ...
            {isImportOpen && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                ...
                <div className="mt-2 text-sm text-green-600">
                  ...
                  center_id (optional), mobile (optional), roles
                  (comma-separated)
                </div>
              </div>
            )}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <CustomModelForm
                  ...
                  onSubmit={async (data, setErrors) => {
                    try {
                      await UsersModelFormConfig.onSubmit(
                        data,
                        mode,
                        selectedUser?.id,
                        setErrors
