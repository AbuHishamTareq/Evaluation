...
import { QuestionsTableConfig } from '../config/tables/question-table';
import api from '../axios';
import { CustomModelForm } from '../components/CustomModelForm';
import { QuestionsModelFormConfig } from '../config/forms/question-model-form';
...
const Questions = () => {
    ...
    const canPrint = hasPermission(`print-question`, userPermissions);
    const canExport = hasPermission(`export-question`, userPermissions);
    const canImport = hasPermission(`import-question`, userPermissions);
    ...
    const fetchAllQuestionsForPrint = async (): Promise<any[]> => {
        try {
            ...
            const fetchedQuestions = res.data.questions.data;
            return fetchedQuestions;
        } catch (error) {
            console.error('Failed to fetch all questions:', error);
            toast({
                title: "Error",
                description: "Failed to fetch all questions data.",
    ...
    const handlePrintClick = async () => {
        ...
        try {
            ...
            const fetchedQuestions = await fetchAllQuestionsForPrint();

            if (fetchedQuestions.length === 0) {
                toast({
                    title: "Warning",
                    description: "No questions found to print.",
    ...
    const handlePrint = useReactToPrint({
        contentRef: printRef,
        documentTitle: 'All Questions Report',
        onAfterPrint: () => {
            setAllQuestions([]);
        },
    });
    ...
    const handleDownloadTemplate = async () => {
        try {
            const response = await api.get('/api/questions/download-template', {
                responseType: 'blob',
            });
    ...
    const fetchQuestions = async (
        ...
        try {
            ...
            const centerData = res.data.questions;
            setQuestions(centerData.data);
    ...
    // Fetch all questions data for export
    ...
    const handlePageChange = (newPage: number) => {
        ...
        fetchQuestions(newPage, pagination.per_page, search, sortBy, sortDir);
    };

    const handlePerPageChange = (newPerPage: number) => {
        fetchQuestions(1, newPerPage, search, sortBy, sortDir);
    };
    ...
    const handleSort = (column: string) => {
        ...
        setSortBy(column);
        setSortDir(newDir);
        fetchQuestions(pagination.current_page, pagination.per_page, search, column, newDir);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const response = await api.delete(`/api/questions/delete/${id}`);
            toast({
                title: "Deleted",
            ...
            await fetchQuestions(); // refresh the data
        } catch (error: any) {
            toast({
                title: "Error",
    ...
    const openModel = (mode: 'create' | 'view' | 'edit', center?: any) => {
        setMode(mode);
        setSelectedDomain(center || null);
        setModelOpen(true);
    };

    const getFormFieldsWithValues = () => {
        return QuestionsModelFormConfig.fields.map((field) => {
            let value = '';
            ...
            if (selectedDomain) {
                if (field.name === 'domain') {
                    ...
                    value = selectedDomain[field.name] ?? '';
                }
            }
    ...
    const handleToggle = async (id: number, checked: boolean) => {
        await api.put(`/api/questions/${id}/status`, {
    ...
    const handleBulkExportCSV = () => {
        ...
        const selectedQuestions = questions.filter(user => selectedRows.includes(user.id));
        const headers = QuestionsTableConfig.columns
            .filter(col => !['actions', 'status'].includes(col.key))
            .map(col => col.label);

        const rows = selectedQuestions.map(center =>
            QuestionsTableConfig.columns
                .filter(col => !['actions', 'status'].includes(col.key))
                .map(col => {
        ...
        link.setAttribute('download', `selected_questions_${selectedRows.length}.csv`);
        document.body.appendChild(link);
        link.click();
    ...
    const handleBulkExportPDF = () => {
        ...
        const selectedQuestions = questions.filter(center => selectedRows.includes(center.id));
        const doc = new jsPDF();
        const pageWidth = doc.internal.pageSize.getWidth();

        const hiddenColumnKeys = ['actions', 'status'];
        const visibleColumns = QuestionsTableConfig.columns.filter(col => !hiddenColumnKeys.includes(col.key));
        ...
        doc.text(`Selected questions Report (${selectedRows.length} questions)`, pageWidth / 2, 20, { align: 'center' });
        ...
        doc.save(`selected_questions_${selectedRows.length}.pdf`);
    };
    ...
    const handleBulkActivate = async () => {
        ...
        const selectedQuestions = questions.filter(center => selectedRows.includes(center.id));
        const inactiveDomainIds = selectedQuestions
        ...
        try {
            await api.put('/api/questions/bulk-activate', {
                question_ids: selectedRows
            });

            const total = selectedRows.length;
            const updated = inactiveDomainIds.length;
            ...
            toast({
                title: "Error",
                description: error.response?.data?.message || "Failed to activate questions.",
                backgroundColor: "bg-red-600",
                color: "text-white",
                variant: "destructive",
            });
    ...
    const handleBulkDeactivate = async () => {
        ...
        const selectedQuestions = questions.filter(center => selectedRows.includes(center.id));
        const activeDomainIds = selectedQuestions
        ...
        try {
            await api.put('/api/questions/bulk-deactivate', {
                question_ids: selectedRows
            });

            const total = selectedRows.length;
            const updated = activeDomainIds.length;
            const message = updated === total
            ...
            toast({
                title: "Error",
                description: error.response?.data?.message || "Failed to deactivate questions.",
                backgroundColor: "bg-red-600",
                color: "text-white",
                variant: "destructive",
            });
    ...
    const handleExportAllCSV = async () => {
        try {
            ...
            const rows = allQuestions.map((center: any) =>
                QuestionsTableConfig.columns
                    .filter(col => !['actions', 'status'].includes(col.key))
                    .map(col => {
                        const value = center[col.key];
            ...
            const link = document.createElement('a');
            link.href = URL.createObjectURL(blob);
            link.setAttribute('download', `all_questions_${allQuestions.length}.csv`);
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            ...
            toast({
                title: "Success",
                description: `Exported ${allQuestions.length} questions to CSV.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
    ...
    const handleCurrentPageExportCSV = () => {
        ...
        const rows = questions.map(center =>
            QuestionsTableConfig.columns
                .filter(col => !['actions', 'status'].includes(col.key))
                .map(col => {
                    const value = center[col.key];
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.setAttribute('download', `questions_page_${pagination.current_page}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        ...
        toast({
            title: "Success",
            description: `Exported ${questions.length} questions from current page to CSV.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
    ...
    const handleBulkExportExcel = async () => {
        ...
        const worksheet = workbook.addWorksheet("Selected questions");

        // Auto-generate columns from object keys
        const firstRow = selectedQuestions[0];
        ...
        // Add data
        selectedQuestions.forEach(center => {
        ...
        saveAs(blob, `selected_questions_${selectedRows.length}.xlsx`);
    };
    ...
    const handleExportAllExcel = async () => {
        try {
            ...
            const workbook = new ExcelJS.Workbook();
            const worksheet = workbook.addWorksheet('All Questions');
            ...
            allQuestions.forEach((center: any) => {
                worksheet.addRow(center);
            });
            ...
            saveAs(blob, `all_questions_${allQuestions.length}.xlsx`);

            toast({
                title: "Success",
                description: `Exported ${allQuestions.length} questions to Excel.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
    ...
    const handleCurrentPageExportExcel = async () => {
        ...
        saveAs(blob, `questions_page_${pagination.current_page}.xlsx`);

        toast({
            title: "Success",
            description: `Exported ${questions.length} questions from current page to Excel.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
    ...
    const handleExportAllPDF = async () => {
        try {
            ...
            doc.setFontSize(16);
            doc.setFont('helvetica', 'bold');
            doc.text(`All Questions Report (${allQuestions.length} questions)`, pageWidth / 2, 20, { align: 'center' });
            ...
            doc.save(`all_questions_${allQuestions.length}.pdf`);

            toast({
                title: "Success",
                description: `Exported ${allQuestions.length} questions to PDF.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
    ...
    const handleCurrentPageExportPDF = () => {
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text(`Questions Report - Page ${pagination.current_page} (${questions.length} questions)`, pageWidth / 2, 20, { align: 'center' });
        ...
        doc.save(`questions_page_${pagination.current_page}.pdf`);

        toast({
            title: "Success",
            description: `Exported ${questions.length} questions from current page to PDF.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
    ...
    return (
        <Header title={'Evolution\'s Questions'}>
            <div className="min-h-screen bg-white p-6">
                <div className="max-w-7xl mx-auto">
                    <div className="mb-6">
                                                ...
                                                <DropdownMenuSub>
                                                    ...
                                                    <DropdownMenuSubContent>
                                                        ...
                                                        <DropdownMenuItem className="data-[highlighted]:bg-indigo-100 data-[highlighted]:text-black" onClick={handleBulkExportCSV}>
                                                            Selected Question{selectedRows.length !== 1 ? 's' : ''}
                                                        </DropdownMenuItem>
                                                    </DropdownMenuSubContent>
                                                </DropdownMenuSub>
                                                <DropdownMenuSub>
                        ...
                        {/* Import Question - Show above search and add button */}
                        {isImportOpen && (
                            <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                                ...
                                <div className="mt-2 text-sm text-green-600">
                                    Upload CSV or Excel files with columns: name, phc_moh_code, domain_id,
                                </div>
                            </div>
                        )}
                        ...
                        <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 print:hidden">
                            ...
                            <div className="relative w-full max-w-md md:flex-1">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        fetchQuestions(1, pagination.per_page, e.target.value, sortBy, sortDir);
                                    }}
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <CustomModelForm
                                    addButton={QuestionsModelFormConfig.addButton}
                                    title={mode === 'view' ? 'View Question' : mode === 'edit' ? 'Update Question' : QuestionsModelFormConfig.title}
                                    description={QuestionsModelFormConfig.description}
                                    fields={getFormFieldsWithValues()}
                                    buttons={QuestionsModelFormConfig.buttons}
                                    onSubmit={async (data, setErrors) => {
                                        try {
                                            await QuestionsModelFormConfig.onSubmit(data, mode, selectedDomain?.id, setErrors);
                                            await fetchQuestions();
                                            closeModel();
                                        } catch (error) {
                        ...
                        <div ref={printRef} className="hidden print:block">
                            ...
                            <div className="flex justify-between items-center border-b pb-4 mb-4">
                                ...
                                <div className="text-center w-full -ml-16">
                                    <h1 className="text-2xl font-bold">Question Report</h1>
                            ...
                            {allQuestions.length === 0 && (
                                <div className="text-center py-8">
                                    <p className="text-gray-600">No questions found to display.</p>
                                </div>
                            )}
