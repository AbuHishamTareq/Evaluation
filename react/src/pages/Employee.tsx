...
import { EmployeesTableConfig } from "@/config/tables/employee-table";
import api from "../axios";
// import { CustomModelForm } from "../components/CustomModelForm";
import { useNavigate } from "react-router-dom";
import { EmployeesModelFormConfig } from "@/config/forms/employee-model-form";
...
export const Employee = () => {
  ...
  const canPrint = hasPermission(`print-employee`, userPermissions);
  const canExport = hasPermission(`export-employee`, userPermissions);
  const canImport = hasPermission(`import-employee`, userPermissions);
  ...
  const fetchAllEmployeesForPrint = async (): Promise<any[]> => {
    try {
      ...
      const fetchedEmployees = res.data.employees.data;
  ...
  const handlePrintClick = async () => {
    ...
    try {
      ...
      const fetchedEmployees = await fetchAllEmployeesForPrint();
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: "All Employees Report",
    onAfterPrint: () => {
      setAllEmployees([]);
    },
  });
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await api.get("/api/employees/download-template", {
        responseType: "blob",
      });
  ...
  const fetchEmployees = async (
    ...
    try {
      ...
      setPagination({
        ...
        last_page: employeeData.last_page,
  ...
  // Fetch all employees data for export
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchEmployees(newPage, pagination.per_page, search, sortBy, sortDir);
  };

  const handlePerPageChange = (newPerPage: number) => {
    fetchEmployees(1, newPerPage, search, sortBy, sortDir);
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      await fetchEmployees(); // refresh the data
    } catch (error: any) {
      toast({
        title: "Error",
        description:
  ...
  //   const openModel = (mode: "create" | "view" | "edit", employee?: any) => {
  //     setMode(mode);
  //     setSelectedCenter(employee || null);
  //     setModelOpen(true);
  //   };

  //   const getFormFieldsWithValues = () => {
  //     return EmployeesModelFormConfig.fields.map((field) => {
  //       let value = "";

  //       if (selectedCenter) {
  //         if (field.name === "employee") {
  //           value = String(selectedCenter.employee_id) || ""; // Pass employee_id as string
  //         } else {
  ...
  const handleBulkExportCSV = () => {
    ...
    const headers = EmployeesTableConfig.columns
      .filter((col) => !["actions", "status"].includes(col.key))
      .map((col) => col.label);

    const rows = selectedEmployee.map((employee) =>
      EmployeesTableConfig.columns
        .filter((col) => !["actions", "status"].includes(col.key))
        .map((col) => {
  ...
  const handleBulkExportPDF = () => {
    ...
    const visibleColumns = EmployeesTableConfig.columns.filter(
  ...
  const handleExportAllCSV = async () => {
    try {
      ...
      const rows = allEmployees.map((employee: any) =>
        EmployeesTableConfig.columns
          .filter((col) => !["actions", "status"].includes(col.key))
          .map((col) => {
            const value = employee[col.key];
      ...
      link.setAttribute(
        "download",
        `all_healthcare_fields_${allEmployees.length}.csv`
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      ...
      toast({
        title: "Success",
        description: `Exported ${allEmployees.length} healthcare fields to CSV.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `employees_page_${pagination.current_page}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    toast({
      title: "Success",
      description: `Exported ${employees.length} fields from current page to CSV.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleBulkExportExcel = async () => {
    ...
    const selectedDomains = employees.filter((employee) =>
      selectedRows.includes(employee.id)
    );
    ...
    // Add data
    selectedDomains.forEach((employee) => {
    ...
    saveAs(blob, `selected_employees_${selectedRows.length}.xlsx`);
  };

  const handleExportAllExcel = async () => {
    try {
      const allDomains = await fetchAllEmployees();
      ...
      // Auto-generate columns from first employee object
      ...
      allDomains.forEach((employee: any) => {
        worksheet.addRow(employee);
      });
      ...
      saveAs(blob, `all_employees_${allDomains.length}.xlsx`);

      toast({
        title: "Success",
        description: `Exported ${allDomains.length} employees to Excel.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportExcel = async () => {
    ...
    saveAs(blob, `employees_page_${pagination.current_page}.xlsx`);

    toast({
      title: "Success",
      description: `Exported ${employees.length} fields from current page to Excel.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleExportAllPDF = async () => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `All Employees Report (${allEmployees.length} employees)`,
        pageWidth / 2,
        20,
        {
          align: "center",
      ...
      doc.save(`all_employees_${allEmployees.length}.pdf`);

      toast({
        title: "Success",
        description: `Exported ${allEmployees.length} healthcare fields to PDF.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportPDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `Employees Report - Page ${pagination.current_page} (${employees.length} employees)`,
      pageWidth / 2,
      20,
      { align: "center" }
    );
    ...
    toast({
      title: "Success",
      description: `Exported ${employees.length} healthcare fields from current page to PDF.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleBulkActivate = async () => {
    ...
    const inactiveCenterIds = selectedEmployees
    ...
    if (inactiveCenterIds.length === 0) {
      toast({
        title: "Notice",
        description: `All selected employee are already active.`,
        backgroundColor: "bg-yellow-600",
        color: "text-white",
      });
      return;
    ...
    try {
      await api.put("/api/employees/bulk-activate", {
      ...
      const message =
        updated === total
          ...
          : `${updated} of ${total} selected employee${
              total > 1 ? "s" : ""
            } activated successfully.`;
      ...
      toast({
        title: "Error",
        description:
          error.response?.data?.message || "Failed to activate employees.",
        backgroundColor: "bg-red-600",
        color: "text-white",
        variant: "destructive",
      });
  ...
  const handleBulkDeactivate = async () => {
    ...
    const activeCenterIds = selectedEmployees
    ...
    if (activeCenterIds.length === 0) {
      toast({
        title: "Notice",
        description: `All selected employee are already inactive.`,
        backgroundColor: "bg-yellow-600",
        color: "text-white",
      });
      return;
    ...
    try {
      await api.put("/api/employees/bulk-deactivate", {
        employee_ids: selectedRows,
      });
      ...
      toast({
        title: "Error",
        description:
          error.response?.data?.message || "Failed to deactivate employees.",
        backgroundColor: "bg-red-600",
        color: "text-white",
        variant: "destructive",
      });
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            ...
            {isImportOpen && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                ...
                <div className="mt-2 text-sm text-green-600">
                  Upload CSV or Excel files with columns: en_employee,
                  ar_employee.
                </div>
              </div>
            )}
            ...
            <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              {hasPermission("create-employee", userPermissions) && (
                <Button
                  id={EmployeesModelFormConfig.addButton.id}
                  key={EmployeesModelFormConfig.addButton.key}
                  onClick={() => navigate("/employees/create")}
                  className={EmployeesModelFormConfig.addButton.className}
                  variant={EmployeesModelFormConfig.addButton.variant}
                >
                  {EmployeesModelFormConfig.addButton.icon && (
                    <EmployeesModelFormConfig.addButton.icon className="w-4 h-4 mr-2" />
                  )}
                  {EmployeesModelFormConfig.addButton.label}
                </Button>
              )}
            </div>
            ...
            <div ref={printRef} className="hidden print:block">
              ...
              <div className="flex justify-between items-center border-b pb-4 mb-4">
                ...
                <div className="text-center w-full -ml-16">
                  <h1 className="text-2xl font-bold">Employees Report</h1>
            ...
            <div className="print:hidden">
              <CustomTable
                ...
                // onView={(employee) => openModel("view", employee)}
                // onEdit={(employee) => openModel("edit", employee)}
                isModel={true}
                sortBy={sortBy}
                sortDir={sortDir}
