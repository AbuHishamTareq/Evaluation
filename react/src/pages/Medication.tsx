...
import { MedicationsTableConfig } from '../config/tables/medication-table';
import api from '../axios';
import { CustomModelForm } from '../components/CustomModelForm';
import { MedicationsModelFormConfig } from '../config/forms/medication-model-form';
...
const Medication = () => {
    ...
    const canPrint = hasPermission(`print-medication`, userPermissions);
    const canExport = hasPermission(`export-medication`, userPermissions);
    const canImport = hasPermission(`import-medication`, userPermissions);
    ...
    const fetchAllMedicationsForPrint = async (): Promise<any[]> => {
        try {
            ...
            const fetchedMedications = res.data.medications.data;
            return fetchedMedications;
        } catch (error) {
            console.error('Failed to fetch all medications:', error);
            toast({
                title: "Error",
                description: "Failed to fetch all medications data.",
    ...
    const handlePrintClick = async () => {
        ...
        try {
            ...
            const fetchedMedications = await fetchAllMedicationsForPrint();

            if (fetchedMedications.length === 0) {
                toast({
                    title: "Warning",
                    description: "No medications found to print.",
    ...
    const handlePrint = useReactToPrint({
        contentRef: printRef,
        documentTitle: 'All Medications Report',
        onAfterPrint: () => {
            setAllMedications([]);
        },
    });
    ...
    const handleImport = async (file: File) => {
        ...
        try {
            ...
            const response = await api.post('/api/medications/import', formData, {
    ...
    const handleDownloadTemplate = async () => {
        try {
            const response = await api.get('/api/medications/download-template', {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'medications_import_template.xlsx');
    ...
    const fetchMedications = async (
        ...
        try {
            ...
            const medicationData = res.data.medications;
            ...
            setPagination({
                current_page: medicationData.current_page,
                last_page: medicationData.last_page,
                per_page: medicationData.per_page,
            });
        } catch {
    ...
    // Fetch all medications data for export
    const fetchAllMedications = async () => {
        try {
            ...
            return res.data.medications.data;
        } catch (error) {
            console.error('Failed to fetch all medications:', error);
            toast({
                title: "Error",
    ...
    const handlePageChange = (newPage: number) => {
        ...
        fetchMedications(newPage, pagination.per_page, search, sortBy, sortDir);
    };

    const handlePerPageChange = (newPerPage: number) => {
        fetchMedications(1, newPerPage, search, sortBy, sortDir);
    };
    ...
    const handleSort = (column: string) => {
        ...
        setSortBy(column);
        setSortDir(newDir);
        fetchMedications(pagination.current_page, pagination.per_page, search, column, newDir);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const response = await api.delete(`/api/medications/delete/${id}`);
            toast({
                title: "Deleted",
            ...
            await fetchMedications(); // refresh the data
        } catch (error: any) {
            toast({
                title: "Error",
                description: error.response?.data?.message || "Failed to delete the medication.",
                backgroundColor: "bg-red-600",
                color: "text-white",
                variant: "destructive",
            });
    ...
    const openModel = (mode: 'create' | 'view' | 'edit', medication?: any) => {
        setMode(mode);
        setSelectedSection(medication || null);
        setSelectedDomain(medication || null);
        setModelOpen(true);
    };

    const getFormFieldsWithValues = () => {
        return MedicationsModelFormConfig.fields.map((field) => {
            let value = '';

            if (field.name === 'section' && selectedSection) {
                value = String(selectedSection.section_id) || '';
            } else if (field.name === 'domain' && selectedDomain) {
                value = String(selectedDomain.domain_id) || '';
            } else {
                // Prefer section first if available, then domain
                value = selectedSection?.[field.name] ?? selectedDomain?.[field.name] ?? '';
            }
    ...
    const handleBulkExportCSV = () => {
        ...
        const selectedMedications = medications.filter(user => selectedRows.includes(user.id));
        const headers = MedicationsTableConfig.columns
            .filter(col => !['actions', 'status'].includes(col.key))
            .map(col => col.label);

        const rows = selectedMedications.map(medication =>
            MedicationsTableConfig.columns
                .filter(col => !['actions', 'status'].includes(col.key))
                .map(col => {
                    const value = medication[col.key];
        ...
        link.setAttribute('download', `selected_medications_${selectedRows.length}.csv`);
        document.body.appendChild(link);
        link.click();
    ...
    const handleBulkExportPDF = () => {
        ...
        const selectedMedications = medications.filter(medication => selectedRows.includes(medication.id));
        const doc = new jsPDF();
        const pageWidth = doc.internal.pageSize.getWidth();

        const hiddenColumnKeys = ['actions', 'status'];
        const visibleColumns = MedicationsTableConfig.columns.filter(col => !hiddenColumnKeys.includes(col.key));
        ...
        doc.text(`Selected medications Report (${selectedRows.length} medications)`, pageWidth / 2, 20, { align: 'center' });
        ...
        doc.save(`selected_medications_${selectedRows.length}.pdf`);
    };
    ...
    const handleExportAllCSV = async () => {
        try {
            ...
            const rows = allMedications.map((medication: any) =>
                MedicationsTableConfig.columns
                    .filter(col => !['actions', 'status'].includes(col.key))
                    .map(col => {
                        const value = medication[col.key];
            ...
            const link = document.createElement('a');
            link.href = URL.createObjectURL(blob);
            link.setAttribute('download', `all_medications_${allMedications.length}.csv`);
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            ...
            toast({
                title: "Success",
                description: `Exported ${allMedications.length} medications to CSV.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
    ...
    const handleCurrentPageExportCSV = () => {
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.setAttribute('download', `medications_page_${pagination.current_page}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        ...
        toast({
            title: "Success",
            description: `Exported ${medications.length} medications from current page to CSV.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
    ...
    const handleBulkExportExcel = async () => {
        ...
        const selectedDomains = medications.filter(medication => selectedRows.includes(medication.id));

        // Create workbook and worksheet
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet("Selected medications");
        ...
        // Add data
        selectedDomains.forEach(medication => {
            worksheet.addRow(medication);
        });
        ...
        saveAs(blob, `selected_medications_${selectedRows.length}.xlsx`);
    };

    const handleExportAllExcel = async () => {
        try {
            const allDomains = await fetchAllMedications();
            ...
            // Auto-generate columns from first medication object
            ...
            allDomains.forEach((medication: any) => {
                worksheet.addRow(medication);
            });
            ...
            saveAs(blob, `all_medications_${allDomains.length}.xlsx`);

            toast({
                title: "Success",
                description: `Exported ${allDomains.length} medications to Excel.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
            ...
            toast({
                title: "Error",
                description: "Failed to export medications.",
                backgroundColor: "bg-red-600",
                color: "text-white",
                variant: "destructive",
            });
    ...
    const handleCurrentPageExportExcel = async () => {
        ...
        saveAs(blob, `medications_page_${pagination.current_page}.xlsx`);

        toast({
            title: "Success",
            description: `Exported ${medications.length} medications from current page to Excel.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
    ...
    const handleExportAllPDF = async () => {
        try {
            ...
            doc.setFontSize(16);
            doc.setFont('helvetica', 'bold');
            doc.text(`All Medications Report (${allMedications.length} medications)`, pageWidth / 2, 20, { align: 'center' });
            ...
            doc.save(`all_medications_${allMedications.length}.pdf`);

            toast({
                title: "Success",
                description: `Exported ${allMedications.length} medications to PDF.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
    ...
    const handleCurrentPageExportPDF = () => {
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text(`Medications Report - Page ${pagination.current_page} (${medications.length} medications)`, pageWidth / 2, 20, { align: 'center' });
        ...
        doc.save(`medications_page_${pagination.current_page}.pdf`);

        toast({
            title: "Success",
            description: `Exported ${medications.length} medications from current page to PDF.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
                    ...
                    <div className="mb-6">
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-medium text-blue-800">
                                        {selectedRows.length} Medication{selectedRows.length !== 1 ? 's' : ''} selected
                                    </span>
                                </div>
                                                ...
                                                <DropdownMenuSub>
                                                    ...
                                                    <DropdownMenuSubContent>
                                                        ...
                                                        <DropdownMenuItem className="data-[highlighted]:bg-indigo-100 data-[highlighted]:text-black" onClick={handleBulkExportCSV}>
                                                            Selected Medication{selectedRows.length !== 1 ? 's' : ''}
                                                        </DropdownMenuItem>
                                                    </DropdownMenuSubContent>
                                                </DropdownMenuSub>
                                                <DropdownMenuSub>
                        ...
                        {isImportOpen && (
                            <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                                ...
                                <div className="mt-2 text-sm text-green-600">
                                    Upload CSV or Excel files with columns: drug_name, allocation, standard_quantity, section_id,
                                </div>
                            </div>
                        )}
                        ...
                        <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 print:hidden">
                            ...
                            <div className="relative w-full max-w-md md:flex-1">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        fetchMedications(1, pagination.per_page, e.target.value, sortBy, sortDir);
                                    }}
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <CustomModelForm
                                    addButton={MedicationsModelFormConfig.addButton}
                                    title={mode === 'view' ? 'View Center' : mode === 'edit' ? 'Update Center' : MedicationsModelFormConfig.title}
                                    description={MedicationsModelFormConfig.description}
                                    fields={getFormFieldsWithValues()}
                                    buttons={MedicationsModelFormConfig.buttons}
                                    onSubmit={async (data, setErrors) => {
                                        try {
                                            await MedicationsModelFormConfig.onSubmit(data, mode, selectedSection?.id, setErrors);
                                            await fetchMedications();
                                            closeModel();
                                        } catch (error) {
                        ...
                        <div ref={printRef} className="hidden print:block">
                                                    ...
                                                    {MedicationsTableConfig.columns
                                                        ...
                                                        .map((column) => (
                                                            <td key={column.key} className="border border-gray-300 px-4 py-2">
                                                                {(() => {
                                                                    const value = medication[column.key];
                                                                    if (Array.isArray(value)) {
                                                                        return value.map(v => v.label || v.name || JSON.stringify(v)).join(', ');
                                                                    }
                                                                    if (typeof value === 'object' && value !== null) {
                            ...
                            {allMedications.length === 0 && (
                                <div className="text-center py-8">
                                    <p className="text-gray-600">No medications found to display.</p>
                                </div>
                            )}
                        ...
                        <div className="print:hidden">
                            <CustomTable
                                columns={MedicationsTableConfig.columns}
                                actions={MedicationsTableConfig.actions}
                                data={medications}
                                ...
                                onView={(medication) => openModel('view', medication)}
                                onEdit={(medication) => openModel('edit', medication)}
                                isModel={true}
                                sortBy={sortBy}
                                sortDir={sortDir}
