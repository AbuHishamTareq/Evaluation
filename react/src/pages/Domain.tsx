...
import { DomainsTableConfig } from '../config/tables/domain-table';
import api from '../axios';
import { CustomModelForm } from '../components/CustomModelForm';
import { DomainsModelFormConfig } from '../config/forms/domain-model-form';
...
const Domains = () => {
    ...
    const canExport = hasPermission(`export-domain`, userPermissions);
    const canImport = hasPermission(`import-domain`, userPermissions);
    ...
    const fetchAllDomainsForPrint = async (): Promise<any[]> => {
        try {
            ...
            const fetchedDomains = res.data.domains.data;
            return fetchedDomains;
        } catch (error) {
            console.error('Failed to fetch all domains:', error);
            toast({
                title: "Error",
                description: "Failed to fetch all domains data.",
    ...
    const handlePrintClick = async () => {
        ...
        try {
            ...
            const fetchedDomains = await fetchAllDomainsForPrint();

            if (fetchedDomains.length === 0) {
                toast({
                    title: "Warning",
                    description: "No domains found to print.",
    ...
    const handlePrint = useReactToPrint({
        contentRef: printRef,
        documentTitle: 'All Domains Report',
        onAfterPrint: () => {
            setAllDomains([]);
        },
    });
    ...
    const handleImport = async (file: File) => {
        ...
        try {
            ...
            const response = await api.post('/api/domains/import', formData, {
    ...
    const handleDownloadTemplate = async () => {
        try {
            const response = await api.get('/api/domains/download-template', {
                responseType: 'blob',
            });
    ...
    const fetchDomains = async (
        ...
        try {
            ...
            const centerData = res.data.domains;
            setDomains(centerData.data);
    ...
    // Fetch all domains data for export
    ...
    const handlePageChange = (newPage: number) => {
        ...
        fetchDomains(newPage, pagination.per_page, search, sortBy, sortDir);
    };

    const handlePerPageChange = (newPerPage: number) => {
        fetchDomains(1, newPerPage, search, sortBy, sortDir);
    };
    ...
    const handleSort = (column: string) => {
        ...
        setSortBy(column);
        setSortDir(newDir);
        fetchDomains(pagination.current_page, pagination.per_page, search, column, newDir);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            await fetchDomains(); // refresh the data
        } catch (error: any) {
            toast({
                title: "Error",
    ...
    const getFormFieldsWithValues = () => {
        return DomainsModelFormConfig.fields.map((field) => {
            let value = '';

            if (selectedSection) {
                if (field.name === 'section') {
                    value = String(selectedSection.section_id) || ''; // Pass section_id as string
                } else {
    ...
    const handleToggle = async (id: number, checked: boolean) => {
        await api.put(`/api/domains/${id}/status`, {
    ...
    const handleBulkExportCSV = () => {
        ...
        const selectedDomains = domains.filter(user => selectedRows.includes(user.id));
        const headers = DomainsTableConfig.columns
            .filter(col => !['actions', 'status'].includes(col.key))
            .map(col => col.label);

        const rows = selectedDomains.map(center =>
            DomainsTableConfig.columns
                .filter(col => !['actions', 'status'].includes(col.key))
                .map(col => {
        ...
        link.setAttribute('download', `selected_domains_${selectedRows.length}.csv`);
        document.body.appendChild(link);
        link.click();
    ...
    const handleBulkExportPDF = () => {
        ...
        const selectedDomains = domains.filter(center => selectedRows.includes(center.id));
        const doc = new jsPDF();
        const pageWidth = doc.internal.pageSize.getWidth();

        const hiddenColumnKeys = ['actions', 'status'];
        const visibleColumns = DomainsTableConfig.columns.filter(col => !hiddenColumnKeys.includes(col.key));
        ...
        doc.text(`Selected domains Report (${selectedRows.length} domains)`, pageWidth / 2, 20, { align: 'center' });
        ...
        doc.save(`selected_domains_${selectedRows.length}.pdf`);
    };
    ...
    const handleBulkActivate = async () => {
        ...
        const selectedDomains = domains.filter(center => selectedRows.includes(center.id));
        const inactiveDomainIds = selectedDomains
        ...
        try {
            await api.put('/api/domains/bulk-activate', {
            ...
            toast({
                title: "Error",
                description: error.response?.data?.message || "Failed to activate domains.",
                backgroundColor: "bg-red-600",
                color: "text-white",
                variant: "destructive",
            });
    ...
    const handleBulkDeactivate = async () => {
        ...
        const selectedDomains = domains.filter(center => selectedRows.includes(center.id));
        const activeDomainIds = selectedDomains
        ...
        try {
            await api.put('/api/domains/bulk-deactivate', {
                center_ids: selectedRows
            });
            ...
            toast({
                title: "Error",
                description: error.response?.data?.message || "Failed to deactivate domains.",
                backgroundColor: "bg-red-600",
                color: "text-white",
                variant: "destructive",
            });
    ...
    const handleExportAllCSV = async () => {
        try {
            ...
            const rows = allDomains.map((center: any) =>
                DomainsTableConfig.columns
                    .filter(col => !['actions', 'status'].includes(col.key))
                    .map(col => {
                        const value = center[col.key];
            ...
            const link = document.createElement('a');
            link.href = URL.createObjectURL(blob);
            link.setAttribute('download', `all_domains_${allDomains.length}.csv`);
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            ...
            toast({
                title: "Success",
                description: `Exported ${allDomains.length} domains to CSV.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
    ...
    const handleCurrentPageExportCSV = () => {
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.setAttribute('download', `domains_page_${pagination.current_page}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        ...
        toast({
            title: "Success",
            description: `Exported ${domains.length} domains from current page to CSV.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
    ...
    const handleBulkExportExcel = async () => {
            ...
            const worksheet = workbook.addWorksheet("Selected domains");
            ...
            saveAs(blob, `selected_domains_${selectedRows.length}.xlsx`);
        };
        ...
        const handleExportAllExcel = async () => {
            try {
                ...
                saveAs(blob, `all_domains_${allDomains.length}.xlsx`);
    
                toast({
                    title: "Success",
                    description: `Exported ${allDomains.length} domains to Excel.`,
                    backgroundColor: "bg-green-600",
                    color: "text-white",
                });
            } catch (error) {
        ...
        const handleCurrentPageExportExcel = async () => {
            ...
            saveAs(blob, `domains_page_${pagination.current_page}.xlsx`);
    
            toast({
                title: "Success",
                description: `Exported ${domains.length} domains from current page to Excel.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        };
    ...
    const handleExportAllPDF = async () => {
        try {
            ...
            doc.setFontSize(16);
            doc.setFont('helvetica', 'bold');
            doc.text(`All Domains Report (${allDomains.length} domains)`, pageWidth / 2, 20, { align: 'center' });
            ...
            doc.save(`all_domains_${allDomains.length}.pdf`);

            toast({
                title: "Success",
                description: `Exported ${allDomains.length} domains to PDF.`,
                backgroundColor: "bg-green-600",
                color: "text-white",
            });
        } catch (error) {
    ...
    const handleCurrentPageExportPDF = () => {
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text(`Domains Report - Page ${pagination.current_page} (${domains.length} domains)`, pageWidth / 2, 20, { align: 'center' });
        ...
        doc.save(`domains_page_${pagination.current_page}.pdf`);

        toast({
            title: "Success",
            description: `Exported ${domains.length} domains from current page to PDF.`,
            backgroundColor: "bg-green-600",
            color: "text-white",
        });
    };
    ...
    return (
        <Header title={'Question\'s Domain'}>
                    ...
                    <div className="mb-6">
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-medium text-blue-800">
                                        {selectedRows.length} Domain{selectedRows.length !== 1 ? 's' : ''} selected
                                    </span>
                                </div>
                                                ...
                                                <DropdownMenuSub>
                                                    ...
                                                    <DropdownMenuSubContent>
                                                        ...
                                                        <DropdownMenuItem className="data-[highlighted]:bg-indigo-100 data-[highlighted]:text-black" onClick={handleBulkExportCSV}>
                                                            Selected Domain{selectedRows.length !== 1 ? 's' : ''}
                                                        </DropdownMenuItem>
                                                    </DropdownMenuSubContent>
                                                </DropdownMenuSub>
                                                <DropdownMenuSub>
                        ...
                        {isImportOpen && (
                            <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                                ...
                                <div className="mt-2 text-sm text-green-600">
                                    Upload CSV or Excel files with columns: name, phc_moh_code, section_id,
                                </div>
                            </div>
                        )}
                        ...
                        <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 print:hidden">
                            ...
                            <div className="relative w-full max-w-md md:flex-1">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        fetchDomains(1, pagination.per_page, e.target.value, sortBy, sortDir);
                                    }}
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <CustomModelForm
                                    addButton={DomainsModelFormConfig.addButton}
                                    title={mode === 'view' ? 'View Domain' : mode === 'edit' ? 'Update Domain' : DomainsModelFormConfig.title}
                                    description={DomainsModelFormConfig.description}
                                    fields={getFormFieldsWithValues()}
                                    buttons={DomainsModelFormConfig.buttons}
                                    onSubmit={async (data, setErrors) => {
                                        try {
                                            await DomainsModelFormConfig.onSubmit(data, mode, selectedSection?.id, setErrors);
                                            await fetchDomains();
                                            closeModel();
                                        } catch (error) {
                        ...
                        <div ref={printRef} className="hidden print:block">
                            ...
                            <div className="flex justify-between items-center border-b pb-4 mb-4">
                                ...
                                <div className="text-center w-full -ml-16">
                                    <h1 className="text-2xl font-bold">Domain Report</h1>
                            ...
                            {allDomains.length === 0 && (
                                <div className="text-center py-8">
                                    <p className="text-gray-600">No domains found to display.</p>
                                </div>
                            )}
                        ...
                        <div className="print:hidden">
                            <CustomTable
                                columns={DomainsTableConfig.columns}
                                actions={DomainsTableConfig.actions}
                                data={domains}
