...
import { SpecialtiesTableConfig } from "@/config/tables/specialty-table";
import api from "../axios";
import { CustomModelForm } from "../components/CustomModelForm";
import { SpecialtiesModelFormConfig } from "@/config/forms/specialty-model-form";
...
export const Specialty = () => {
  ...
  const canPrint = hasPermission(`print-specialty`, userPermissions);
  const canExport = hasPermission(`export-specialty`, userPermissions);
  const canImport = hasPermission(`import-specialty`, userPermissions);
  ...
  const fetchAllSpecialtiesForPrint = async (): Promise<any[]> => {
    try {
      ...
      const fetchedSpecialties = res.data.specialties.data;
  ...
  const handlePrintClick = async () => {
    ...
    try {
      ...
      const fetchedSpecialties = await fetchAllSpecialtiesForPrint();
      ...
      setAllSpecialties(fetchedSpecialties);
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      const response = await api.post("/api/specialties/import", formData, {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await api.get("/api/specialties/download-template", {
        responseType: "blob",
      });
  ...
  const fetchSpecialties = async (
    ...
    try {
      ...
      const specialtyData = res.data.specialties;
      setSpecialties(specialtyData.data);

      setPagination({
        current_page: specialtyData.current_page,
        last_page: specialtyData.last_page,
        per_page: specialtyData.per_page,
      });
    } catch {
  ...
  // Fetch all specialties data for export
  const fetchAllSpecialties = async () => {
    try {
      ...
      return res.data.specialties.data;
    } catch (error) {
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchSpecialties(newPage, pagination.per_page, search, sortBy, sortDir);
  };

  const handlePerPageChange = (newPerPage: number) => {
    fetchSpecialties(1, newPerPage, search, sortBy, sortDir);
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await api.delete(`/api/specialties/delete/${id}`);
      toast({
        title: "Deleted",
        description:
      ...
      await fetchSpecialties(); // refresh the data
    } catch (error: any) {
      toast({
        title: "Error",
        description:
  ...
  const openModel = (mode: "create" | "view" | "edit", specialty?: any) => {
    setMode(mode);
    setSelectedCenter(specialty || null);
    setModelOpen(true);
  };

  const getFormFieldsWithValues = () => {
    return SpecialtiesModelFormConfig.fields.map((field) => {
      let value = "";
  ...
  const handleBulkExportCSV = () => {
    ...
    const selectedSpecialty = specialties.filter((specialty) =>
      selectedRows.includes(specialty.id)
    );
    const headers = SpecialtiesTableConfig.columns
      .filter((col) => !["actions", "status"].includes(col.key))
      .map((col) => col.label);

    const rows = selectedSpecialty.map((specialty) =>
      SpecialtiesTableConfig.columns
        .filter((col) => !["actions", "status"].includes(col.key))
        .map((col) => {
          const value = specialty[col.key];
  ...
  const handleBulkExportPDF = () => {
    ...
    const visibleColumns = SpecialtiesTableConfig.columns.filter(
  ...
  const handleExportAllCSV = async () => {
    try {
      const allSpecialties = await fetchAllSpecialties();
      ...
      const rows = allSpecialties.map((specialty: any) =>
        SpecialtiesTableConfig.columns
          .filter((col) => !["actions", "status"].includes(col.key))
          .map((col) => {
            const value = specialty[col.key];
      ...
      link.setAttribute(
        "download",
        `all_healthcare_fields_${allSpecialties.length}.csv`
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      ...
      toast({
        title: "Success",
        description: `Exported ${allSpecialties.length} healthcare fields to CSV.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `specialties_page_${pagination.current_page}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    toast({
      title: "Success",
      description: `Exported ${specialties.length} fields from current page to CSV.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleBulkExportExcel = async () => {
    ...
    const selectedDomains = specialties.filter((specialty) =>
      selectedRows.includes(specialty.id)
    );
    ...
    // Add data
    selectedDomains.forEach((specialty) => {
      worksheet.addRow(specialty);
    });
    ...
    saveAs(blob, `selected_specialties_${selectedRows.length}.xlsx`);
  };

  const handleExportAllExcel = async () => {
    try {
      const allDomains = await fetchAllSpecialties();
      ...
      // Auto-generate columns from first specialty object
      ...
      // Add each specialty as a row
      allDomains.forEach((specialty: any) => {
        worksheet.addRow(specialty);
      });
      ...
      saveAs(blob, `all_specialties_${allDomains.length}.xlsx`);

      toast({
        title: "Success",
        description: `Exported ${allDomains.length} specialties to Excel.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to export specialties.",
        backgroundColor: "bg-red-600",
        color: "text-white",
        variant: "destructive",
      });
  ...
  const handleCurrentPageExportExcel = async () => {
    ...
    saveAs(blob, `specialties_page_${pagination.current_page}.xlsx`);

    toast({
      title: "Success",
      description: `Exported ${specialties.length} fields from current page to Excel.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleExportAllPDF = async () => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `All Specialties Report (${allSpecialties.length} specialties)`,
        pageWidth / 2,
        20,
        {
          align: "center",
      ...
      doc.save(`all_specialties_${allSpecialties.length}.pdf`);

      toast({
        title: "Success",
        description: `Exported ${allSpecialties.length} healthcare fields to PDF.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportPDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `Specialties Report - Page ${pagination.current_page} (${specialties.length} specialties)`,
      pageWidth / 2,
      20,
      { align: "center" }
    );
    ...
    toast({
      title: "Success",
      description: `Exported ${specialties.length} healthcare fields from current page to PDF.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  return (
    <Header title={"Healthcare Specialties Management"}>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-blue-800">
                    {selectedRows.length} Healthcare specialtie
            ...
            {isImportOpen && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                ...
                <div className="mt-2 text-sm text-green-600">
                  Upload CSV or Excel files with columns: en_specialty, ar_specialty.
                </div>
              </div>
            )}
            ...
            <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <CustomModelForm
                addButton={SpecialtiesModelFormConfig.addButton}
                title={
                  mode === "view"
                    ? "View Healthcare Field"
                    : mode === "edit"
                    ? "Update Healthcare Field"
                    : SpecialtiesModelFormConfig.title
                }
                description={SpecialtiesModelFormConfig.description}
                fields={getFormFieldsWithValues()}
                buttons={SpecialtiesModelFormConfig.buttons}
                onSubmit={async (data, setErrors) => {
                  try {
                    await SpecialtiesModelFormConfig.onSubmit(
                      data,
                      mode,
                      selectedCenter?.id,
                      setErrors
            ...
            <div ref={printRef} className="hidden print:block">
                        ...
                        >
                          {SpecialtiesTableConfig.columns
                            ...
                            .map((column) => (
                              ...
                              >
                                {(() => {
                                  const value = specialty[column.key];
                                  if (Array.isArray(value)) {
                                    return value
                                      .map(
                                        (v) =>
              ...
              {allSpecialties.length === 0 && (
                <div className="text-center py-8">
                  <p className="text-gray-600">No specialties found to display.</p>
                </div>
              )}
            ...
            <div className="print:hidden">
              <CustomTable
                columns={SpecialtiesTableConfig.columns}
                actions={SpecialtiesTableConfig.actions}
                data={specialties}
                ...
                onView={(specialty) => openModel("view", specialty)}
                onEdit={(specialty) => openModel("edit", specialty)}
                isModel={true}
                sortBy={sortBy}
                sortDir={sortDir}
