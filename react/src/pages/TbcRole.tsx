...
import { TbcRolesTableConfig } from "@/config/tables/tbcRole-table";
import api from "../axios";
import { CustomModelForm } from "../components/CustomModelForm";
import { TbcRolesModelFormConfig } from "@/config/forms/tbcRole-model-form";
...
export const TbcRole = () => {
  const [tbcRoles, setTbcRoles] = useState<any[]>([]);
  ...
  const canPrint = hasPermission(`print-tbc-role`, userPermissions);
  const canExport = hasPermission(`export-tbc-role`, userPermissions);
  const canImport = hasPermission(`import-tbc-role`, userPermissions);

  const printRef = useRef<HTMLDivElement>(null);
  const [allTbcRoles, setAllTbcRoles] = useState<any[]>([]);
  const [isLoadingAll, setIsLoadingAll] = useState(false);

  // Fetch all users data for printing - Fixed version
  const fetchAllTbcRolesForPrint = async (): Promise<any[]> => {
    try {
      const res = await api.get(`/api/tbcRoles`, {
        params: {
          page: 1,
      ...
      const fetchedTbcRoles = res.data.tbcRoles.data;
      return fetchedTbcRoles;
    } catch (error) {
  ...
  const handlePrintClick = async () => {
    ...
    try {
      ...
      const fetchedTbcRoles = await fetchAllTbcRolesForPrint();

      if (fetchedTbcRoles.length === 0) {
        toast({
          title: "Warning",
      ...
      setAllTbcRoles(fetchedTbcRoles);
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: "All TbcRoles Report",
    onAfterPrint: () => {
      setAllTbcRoles([]);
    },
  });
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      const response = await api.post("/api/tbcRoles/import", formData, {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await api.get("/api/tbcRoles/download-template", {
        responseType: "blob",
      });
  ...
  const fetchTbcRoles = async (
    ...
    try {
      ...
      const tbcRoleData = res.data.tbcRoles;
      setTbcRoles(tbcRoleData.data);

      setPagination({
        current_page: tbcRoleData.current_page,
        last_page: tbcRoleData.last_page,
        per_page: tbcRoleData.per_page,
      });
    } catch {
  ...
  // Fetch all tbcRoles data for export
  const fetchAllTbcRoles = async () => {
    try {
      const res = await api.get(`/api/tbcRoles`, {
        params: {
          page: 1,
  ...
  const handlePageChange = (newPage: number) => {
    ...
    fetchTbcRoles(newPage, pagination.per_page, search, sortBy, sortDir);
  };

  const handlePerPageChange = (newPerPage: number) => {
    fetchTbcRoles(1, newPerPage, search, sortBy, sortDir);
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await api.delete(`/api/tbcRoles/delete/${id}`);
      toast({
        title: "Deleted",
        description: response.data?.message || "TbcRoles deleted successfully.",
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
      await fetchTbcRoles(); // refresh the data
    } catch (error: any) {
      toast({
        title: "Error",
        description:
  ...
  const openModel = (mode: "create" | "view" | "edit", tbcRole?: any) => {
    setMode(mode);
    setSelectedCenter(tbcRole || null);
    setModelOpen(true);
  };

  const getFormFieldsWithValues = () => {
    return TbcRolesModelFormConfig.fields.map((field) => {
      let value = "";
  ...
  const handleBulkExportCSV = () => {
    ...
    const selectedTbcRole = tbcRoles.filter((tbcRole) =>
      selectedRows.includes(tbcRole.id)
    );
    const headers = TbcRolesTableConfig.columns
      .filter((col) => !["actions", "status"].includes(col.key))
      .map((col) => col.label);

    const rows = selectedTbcRole.map((tbcRole) =>
      TbcRolesTableConfig.columns
        .filter((col) => !["actions", "status"].includes(col.key))
        .map((col) => {
          const value = tbcRole[col.key];
  ...
  const handleBulkExportPDF = () => {
    ...
    const visibleColumns = TbcRolesTableConfig.columns.filter(
  ...
  const handleExportAllCSV = async () => {
    try {
      const allTbcRoles = await fetchAllTbcRoles();

      if (allTbcRoles.length === 0) {
        toast({
          title: "Error",
          description: "You must select at least one Record.",
          backgroundColor: "bg-red-600",
      ...
      const rows = allTbcRoles.map((tbcRole: any) =>
        TbcRolesTableConfig.columns
          .filter((col) => !["actions", "status"].includes(col.key))
          .map((col) => {
            const value = tbcRole[col.key];
      ...
      link.setAttribute(
        "download",
        `all_healthcare_fields_${allTbcRoles.length}.csv`
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      ...
      toast({
        title: "Success",
        description: `Exported ${allTbcRoles.length} healthcare fields to CSV.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportCSV = () => {
    ...
    const rows = tbcRoles.map((tbcRole) =>
      TbcRolesTableConfig.columns
        .filter((col) => !["actions", "status"].includes(col.key))
        .map((col) => {
          const value = tbcRole[col.key];
    ...
    link.setAttribute(
      "download",
      `tbcRoles_page_${pagination.current_page}.csv`
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    toast({
      title: "Success",
      description: `Exported ${tbcRoles.length} fields from current page to CSV.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleBulkExportExcel = async () => {
    ...
    const selectedDomains = tbcRoles.filter((tbcRole) =>
      selectedRows.includes(tbcRole.id)
    );
    ...
    // Add data
    selectedDomains.forEach((tbcRole) => {
      worksheet.addRow(tbcRole);
    });
    ...
    saveAs(blob, `selected_tbcRoles_${selectedRows.length}.xlsx`);
  };

  const handleExportAllExcel = async () => {
    try {
      const allDomains = await fetchAllTbcRoles();
      ...
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("All TbcRoles");

      // Auto-generate columns from first tbcRole object
      ...
      // Add each tbcRole as a row
      allDomains.forEach((tbcRole: any) => {
        worksheet.addRow(tbcRole);
      });
      ...
      saveAs(blob, `all_tbcRoles_${allDomains.length}.xlsx`);

      toast({
        title: "Success",
        description: `Exported ${allDomains.length} tbcRoles to Excel.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to export tbcRoles.",
        backgroundColor: "bg-red-600",
        color: "text-white",
        variant: "destructive",
      });
  ...
  const handleCurrentPageExportExcel = async () => {
    ...
    saveAs(blob, `tbcRoles_page_${pagination.current_page}.xlsx`);

    toast({
      title: "Success",
      description: `Exported ${tbcRoles.length} fields from current page to Excel.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  const handleExportAllPDF = async () => {
    try {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `All TbcRoles Report (${allTbcRoles.length} tbcRoles)`,
        pageWidth / 2,
        20,
        {
          align: "center",
      ...
      doc.save(`all_tbcRoles_${allTbcRoles.length}.pdf`);

      toast({
        title: "Success",
        description: `Exported ${allTbcRoles.length} healthcare fields to PDF.`,
        backgroundColor: "bg-green-600",
        color: "text-white",
      });
    } catch (error) {
  ...
  const handleCurrentPageExportPDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `TbcRoles Report - Page ${pagination.current_page} (${tbcRoles.length} tbcRoles)`,
      pageWidth / 2,
      20,
      { align: "center" }
    );
    ...
    toast({
      title: "Success",
      description: `Exported ${tbcRoles.length} healthcare fields from current page to PDF.`,
      backgroundColor: "bg-green-600",
      color: "text-white",
    });
  };
  ...
  return (
    <Header title={"Team Based Code Role Management"}>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-blue-800">
                    {selectedRows.length} Team Base Code Role
            ...
            {isImportOpen && (
              <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg print:hidden">
                ...
                <div className="mt-2 text-sm text-green-600">
                  Upload CSV or Excel files with columns: en_tbcRole,
                  ar_tbcRole.
                </div>
              </div>
            )}
            ...
            <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <CustomModelForm
                addButton={TbcRolesModelFormConfig.addButton}
                title={
                  mode === "view"
                    ? "View TbcRole"
                    : mode === "edit"
                    ? "Update TbcRole"
                    : TbcRolesModelFormConfig.title
                }
                description={TbcRolesModelFormConfig.description}
                fields={getFormFieldsWithValues()}
                buttons={TbcRolesModelFormConfig.buttons}
                onSubmit={async (data, setErrors) => {
                  try {
                    await TbcRolesModelFormConfig.onSubmit(
                      data,
                      mode,
                      selectedCenter?.id,
                      setErrors
            ...
            <div ref={printRef} className="hidden print:block">
              ...
              <div className="flex justify-between items-center border-b pb-4 mb-4">
                ...
                <div className="text-center w-full -ml-16">
                  <h1 className="text-2xl font-bold">TbcRoles Report</h1>
                  <p className="text-sm text-gray-600">
                    Generated on: {new Date().toLocaleString()}
                  </p>
                </div>
              </div>
              {allTbcRoles.length > 0 && (
                        ...
                        >
                          {TbcRolesTableConfig.columns
                            ...
                            .map((column) => (
                              ...
                              >
                                {(() => {
                                  const value = tbcRole[column.key];
                                  if (Array.isArray(value)) {
                                    return value
                                      .map(
                                        (v) =>
              ...
              {allTbcRoles.length === 0 && (
                <div className="text-center py-8">
                  <p className="text-gray-600">No tbcRoles found to display.</p>
                </div>
              )}
            ...
            <div className="print:hidden">
              <CustomTable
                columns={TbcRolesTableConfig.columns}
                actions={TbcRolesTableConfig.actions}
                data={tbcRoles}
                ...
                onView={(tbcRole) => openModel("view", tbcRole)}
                onEdit={(tbcRole) => openModel("edit", tbcRole)}
                isModel={true}
                sortBy={sortBy}
                sortDir={sortDir}
